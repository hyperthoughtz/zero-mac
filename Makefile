...
_build/$(APP_NAME):
	...
	@cp app/Zero.icns _build/$(APP_NAME)/Contents/Resources/
...
install: build build-app download-ui
	...
	@/bin/rm -rf /Applications/$(APP_NAME) && cp -r _build/$(APP_NAME) /Applications/$(APP_NAME)
	...
	@/bin/rm -rf $(HOME)/.config/mihomo/ui && cp -r _build/ui $(HOME)/.config/mihomo/ui
	@echo "==> Installing LaunchDaemon..."
	@sed "s|CONFIG_DIR|$(HOME)/.config/mihomo|g" pkg/com.zero.mihomo.plist | sudo tee /Library/LaunchDaemons/com.zero.mihomo.plist > /dev/null
	@sudo chown root:wheel /Library/LaunchDaemons/com.zero.mihomo.plist
	@sudo chmod 644 /Library/LaunchDaemons/com.zero.mihomo.plist
	@sudo launchctl bootout system/com.zero.mihomo 2>/dev/null || true
	@sudo launchctl bootstrap system /Library/LaunchDaemons/com.zero.mihomo.plist
	@echo ""
	@echo "==> Installed! mihomo daemon is running. Open Zero from Applications or Spotlight."

# --- Package .pkg for distribution ---

pkg: build build-app download-ui
	@echo "==> Packaging..."
	@/bin/rm -rf _build/pkg-root
	...
	@cp pkg/com.zero.mihomo.plist _build/pkg-root/usr/local/share/mihomo/
	@cp -r _build/ui/* _build/pkg-root/usr/local/share/mihomo/ui/
	@find _build/pkg-root -name '._*' -delete
...
uninstall:
	@echo "==> Uninstalling..."
	@sudo launchctl bootout system/com.zero.mihomo 2>/dev/null || true
	@sudo rm -f /Library/LaunchDaemons/com.zero.mihomo.plist
